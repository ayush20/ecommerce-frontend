...
function TagsProduct({onParentData}) {
  ...
  const [tagsPageData , setTagsPageData] = useState("")
    ...
    if (didMountRef.current) {
      ...
      ApiService.postData("category-detail", dataString).then((res) => {
        if (res.status === "success" ) {
          setTagsPageData(res.resCategory);
        }
      });
    }
  ...
  const applySortBy = (sort) => {
    ApiService.fetchData(
      ...
      if (res.status === "success") {
        setTagsData(res.tagsData);
        setTagName(res.tagsData.tag_name);
        if (res.resProductsData.data.length > 0) {
          setProductsData(res.resProductsData.data);
          setLoading(false);
        } else {
          setLoading(false);
        }
  ...
  return (
    <>
      <Helmet>
      <title>{tagsPageData.page_meta_title}</title>
        <meta name="description" itemprop="description" content={tagsPageData.page_meta_desc != null ? tagsPageData.page_meta_desc :"Momabatti"} />
        {tagsPageData.page_meta_keyword != null ?<meta name="keywords" content={tagsPageData.page_meta_keyword} />:""}
        <link rel="canonical" href={window.location.href} />
        <meta property="og:title" content={tagsPageData.page_meta_title} />
        <meta name="twitter:url" content={window.location.href} />
             <meta property="og:image" content= {constant.FRONT_URL+'img/logo.png'}/>
        <meta property="og:url" content={window.location.href} />
       <meta property="og:description" content={tagsPageData.page_meta_desc != null ?tagsPageData.page_meta_desc:'Momabatti'} />
        <meta name="twitter:title" content={tagsPageData.page_meta_title} />
        <meta name="twitter:description" content={tagsPageData.page_meta_desc != null ?tagsPageData.page_meta_desc:"Momabatti"} />
                ...
                <Col lg={9}>
                  <Breadcrumb>
                    ...
                    <Breadcrumb.Item active>
                      {TagName ? TagName : tagsData.tag_name}
                    </Breadcrumb.Item>
                  </Breadcrumb>
                  ...
                  <div className="shortby">
                    ...
                    <ul className="sort-list">
                      ...
                      <li className={sorting === 'disc' ? 'active': ''} id="disc" onClick={(e)=>applySortBy("disc")}>
                        <a href="javascript:void(0)">
                          Discount
                        </a>
                      </li>
      ...
      <MobileView>
        <MobileHeader
          PageName={TagName ? TagName : tagsData.tag_name}
          Route=""
          cartCount={cartCount}
        />
                                  ...
                                  {multiCurrency(value.product_price)}
                                    ...
                                    {discount > 0 ? (
                                      <>
                                        <del className="old-price">
                                          ₹
                                          {numeral(value.product_price).format(
                                            "0,0.00"
                                          )}
                                        </del>
                                        <span className="off">
